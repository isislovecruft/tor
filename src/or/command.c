...
command_process_create_cell(cell_t *cell, channel_t *chan)
{
  ...
  if (cell->header.circ_id == 0) {
    log_fn(LOG_PROTOCOL_WARN, LD_PROTOCOL,
           "Received a create cell (type %d) from %s with zero circID; "
           " ignoring.", (int)cell->header.command,
           channel_get_actual_remote_descr(chan));
    return;
  }

  if (circuit_id_in_use_on_channel(cell->header.circ_id, chan)) {
  ...
  if (we_are_hibernating()) {
    log_info(LD_OR,
             ...
             "destroy.");
    channel_send_destroy(cell->header.circ_id, chan,
  ...
  if (chan->wide_circ_ids)
    id_is_high = cell->header.circ_id & (1u<<31);
  else
    id_is_high = cell->header.circ_id & (1u<<15);
  if ((id_is_high &&
  ...
  circ = or_circuit_new(cell->header.circ_id, chan);
...
command_process_created_cell(cell_t *cell, channel_t *chan)
{
  ...
  circ = circuit_get_by_circid_channel(cell->header.circ_id, chan);

  if (!circ) {
    log_info(LD_OR,
             "(circID %u) unknown circ (probably got a destroy earlier). "
             "Dropping.", (unsigned)cell->header.circ_id);
    return;
  }

  if (circ->n_circ_id != cell->header.circ_id || circ->n_chan != chan) {
    log_fn(LOG_PROTOCOL_WARN,LD_PROTOCOL,
...
command_process_relay_cell(cell_t *cell, channel_t *chan)
{
  ...
  if (!CIRCUIT_IS_ORIGIN(circ) &&
      ...
      cell->header.circ_id == TO_OR_CIRCUIT(circ)->p_circ_id)
...
command_process_destroy_cell(cell_t *cell, channel_t *chan)
{
  ...
  log_debug(LD_OR,"Received for circID %u.",(unsigned)cell->header.circ_id);
